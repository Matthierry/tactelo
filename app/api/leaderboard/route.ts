...
import { leaderboardRows } from "../../lib/demo-data";
...
export async function GET(request: Request) {
  ...
  if (!env.DB) return Response.json({ scope, rows: leaderboardRows, source: "demo" });
  try {
    ...
    return Response.json({ scope, rows: result.results, source: "database" });
  } catch {
    return Response.json({ scope, rows: leaderboardRows, source: "demo" });
  }
}
